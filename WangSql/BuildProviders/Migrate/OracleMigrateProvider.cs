...
namespace WangSql.BuildProviders.Migrate
{
    public class OracleMigrateProvider : DefaultMigrateProvider, IMigrateProvider
    {
        ...
        {
            ResolveColumnInfo(columnInfo);
            sqlMapper.Execute($"ALTER TABLE {sqlMapper.SqlFactory.DbProvider.FormatQuotationForSql(tableName)} MODIFY {sqlMapper.SqlFactory.DbProvider.FormatQuotationForSql(columnInfo.Name)} type {ResolveDataType(columnInfo)}", null);
        }
        ...
        public override void AlterColumnNull(string tableName, string columnName, bool nullable = false)
        {
            if (nullable)
            {
                sqlMapper.Execute($"ALTER TABLE {sqlMapper.SqlFactory.DbProvider.FormatQuotationForSql(tableName)} MODIFY {sqlMapper.SqlFactory.DbProvider.FormatQuotationForSql(columnName)} NULL", null);
            }
            else
            {
                sqlMapper.Execute($"ALTER TABLE {sqlMapper.SqlFactory.DbProvider.FormatQuotationForSql(tableName)} MODIFY {sqlMapper.SqlFactory.DbProvider.FormatQuotationForSql(columnName)} NOT NULL", null);
            }
        }
        ...
        public override void AddTrigger(string tableName, string procedureName, TriggerPosition triggerPosition, TriggerEvent triggerEvent, string triggerName = null)
        {
            ...
            if (!ExsitTrigger(triggerName))
                sqlMapper.Execute($"CREATE TRIGGER {triggerName} {triggerPosition.ToString()} {triggerEvent.ToString()} ON {sqlMapper.SqlFactory.DbProvider.FormatQuotationForSql(tableName)} FOR EACH ROW BEGIN {procedureName}(); END;", null);
        }
        ...
        public override void DropTrigger(string tableName, string procedureName, string triggerName = null)
        {
            if (string.IsNullOrEmpty(triggerName)) triggerName = $"{tableName}_{procedureName}";
            if (ExsitTrigger(triggerName))
                sqlMapper.Execute($"DROP TRIGGER {triggerName}", null);
        }
        ...
        {
            string sql = $"select count(*) from user_tables where table_name = '{(sqlMapper.SqlFactory.DbProvider.UseQuotationInSql ? tableName : tableName.ToUpper())}'";
        ...
        {
            return sqlMapper.Scalar<int>("select count(1) from user_triggers where trigger_name =#trigger_name#", triggerName) > 0;
        }
