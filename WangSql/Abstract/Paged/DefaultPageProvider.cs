...
namespace WangSql.Abstract.Paged
{
    public class DefaultPageProvider : IPageProvider
    {
        ...
        public virtual IEnumerable<T> QueryPage<T>(string sql, object param, int pageIndex, int pageSize, bool buffered = true, int? timeout = null)
        {
            var r = _sqlMapper.Query<T>(sql, param, buffered, timeout);
            var rr = r.Skip((pageIndex - 1) * pageSize).Take(pageSize);
            return rr;
        }
        ...
        public virtual async Task<IEnumerable<T>> QueryPageAsync<T>(string sql, object param, int pageIndex, int pageSize, bool buffered = true, int? timeout = null)
        {
            var r = await _sqlMapper.QueryAsync<T>(sql, param, buffered, timeout);
            var rr = r.Skip((pageIndex - 1) * pageSize).Take(pageSize);
            return rr;
        }
    }
