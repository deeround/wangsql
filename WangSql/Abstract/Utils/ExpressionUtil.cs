...
namespace WangSql.Abstract.Utils
{
    public class ExpressionUtil : ExpressionVisitor
    {
        ...
        protected override Expression VisitConstant(ConstantExpression node)
        {
            ...
            {
                _build.AppendFormat("'{0}'", node.Value.ToString());
            }
            else
            {
                _build.AppendFormat("{0}", node.Value);
        ...
        public void SetName(MemberExpression expression)
        {
            ...
            var columnName = EntityUtil.GetColumn(expression.Expression.Type, f => f.PropertyName == name)?.ColumnName ?? name;
            if (!_singleTable)
            {
                var tableName = EntityUtil.GetMap(expression.Expression.Type).TableName;
        ...
        public static Dictionary<string, string> BuildColumns(Expression expression, Dictionary<string, object> param, bool singleTable = true)
        {
            ...
            else if (expression is MemberExpression)
            {
                ...
                var columnName = EntityUtil.GetColumn(memberExpression.Expression.Type, f => f.PropertyName == name)?.ColumnName ?? name;
                if (!singleTable)
                {
                    var tableName = EntityUtil.GetMap(memberExpression.Expression.Type).TableName;
                    columnName = string.Format("{0}.{1}", tableName, columnName);
                }
                columns.Add(name, columnName);
            }
